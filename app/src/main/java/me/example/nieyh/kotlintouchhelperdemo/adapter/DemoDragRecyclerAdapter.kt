...
class DemoDragRecyclerAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder?, position: Int) {
        ...
        holder?.let {
            when (holder) {
                //如果是组标题部分
                is GroupViewHolder -> {
                    if (holder.mTitle != null && data is String) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup?, viewType: Int): RecyclerView.ViewHolder {
        when (viewType) {
            TYPE_SUB_IMG -> {
                ...
                parent?. let {
                    val layoutLayouter = LayoutInflater.from(parent.context)
                    val root = layoutLayouter.inflate(R.layout.recycler_group_item, parent, false)
    ...
    class GroupViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        var mTitle: TextView
        init {
            mTitle = itemView.findViewById(R.id.recycler_group_item_title) as TextView
        }
    }
